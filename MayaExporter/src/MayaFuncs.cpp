...
template<class T>
int AddZeroed(std::vector<T>& vec, int n)
{
	int arrayNum = vec.size();
	if(arrayNum + n > vec.capacity())
		vec.reserve(vec.size() + n + 3 * (vec.size() + n) / 8 + INITIALSIZE);

	for(int i = arrayNum;i < vec.size();++i)
	{
...
template<class T>
int AddExactZeroed(std::vector<T>& vec, int n)
{
	int arrayNum = vec.size();
	if(arrayNum + n > vec.capacity())
...
template<class T>
void AddUniqueItem(std::vector<T>& vec, const T& val)
{
	size_t num = 0;
...
template<class T>
int Contains(const std::vector<T>& vec, T val)
{
	int count = 0;
...
void MeshProcessor::createNewSmoothingGroupsFromTriangulatedMesh( triangulatedMesh& mesh )
{
	...
	mVertEdgePools.push_back( VertEdges() );  
	...
	mFacePools.insert(mFacePools.end(), numPolygons, FaceEnviron() );
		...
		for ( int vid=0; vid<3;vid++ ) 
		{
			...
			if ( Edge )
			{
				...
				if( TouchingFaceIdx >= 0)
				{
					if( Edge->smooth )
					{
						AddUniqueItem(mFacePools[p].SmoothFaces, TouchingFaceIdx);
					}
					else
					{
						AddUniqueItem(mFacePools[p].HardFaces, TouchingFaceIdx);				
					}
				}
			}
		}
	...
	AddZeroed(mPolyProcessFlags, numPolygons );
	...
	//
	AddZeroed(mFaceSmoothingGroups, numPolygons );
	int SmoothParts = 0;
	mCurrentGroup = 0;
	...
	//
	AddZeroed(mGroupTouchPools, mCurrentGroup ); 
	for( int f=0; f< numPolygons; f++)
	{
		std::vector<int> TempTouchPool;

		// This face's own groups..
		{for( int i=0; i< mFaceSmoothingGroups[f].Groups.size(); i++)
		{
			AddUniqueItem(TempTouchPool, mFaceSmoothingGroups[f].Groups[i] );
		}}

		// And those of the neighbours, whether touching or not..
		{for( int n=0; n< mFacePools[f].HardFaces.size(); n++)
		{
			int FaceIdx = mFacePools[f].HardFaces[n];
			for( int i=0; i< mFaceSmoothingGroups[ FaceIdx].Groups.size(); i++)
			{
				AddUniqueItem(TempTouchPool, mFaceSmoothingGroups[FaceIdx].Groups[i] );
			}
		}}
		{for( int n=0; n< mFacePools[f].SmoothFaces.size(); n++)
		{
		...
		for( int g=0; g< TempTouchPool.size(); g++)
		{
			for( int t=0; t< TempTouchPool.size(); t++ )
			{
				AddUniqueItem(mGroupTouchPools[ TempTouchPool[g] ].Groups, TempTouchPool[t] );
			}
		}
	}
	...
	int numRawSmoothingGroups = mGroupTouchPools.size();
	int HardClashes = 0;
	int MaxGroup = 0;
	for( int gInit =0; gInit <mGroupTouchPools.size(); gInit++)
	{
		mGroupTouchPools[gInit].FinalGroup = -1;
	}

	for( int g=0; g<mGroupTouchPools.size(); g++ )
	{		
		...
		for( int SGTries=0; SGTries<32; SGTries++ )
		{
			...
			for( int t=0; t< mGroupTouchPools[g].Groups.size(); t++ )
			{
		...
		MaxGroup = std::max( MaxGroup, mGroupTouchPools[g].FinalGroup );
		if( (g!=0) && (( g & 65535 )==0) )
		{
	...
	{
		// This face's own groups..
		for( int i=0; i< mFaceSmoothingGroups[p].Groups.size(); i++)
		{			
	...
	AddExactZeroed(mPolySmoothingGroups, mFaceSmoothingGroups.size());
	{for( int i=0; i< mPolySmoothingGroups.size(); i++)
	{
		// OR-in all the smoothing bits.
		for( int s=0; s< mFaceSmoothingGroups[i].Groups.size(); s++)
		{ 
...
void MeshProcessor::createNewSmoothingGroups( MDagPath& mesh )
{
	...
	mVertEdgePools.clear();
	AddZeroed(mVertEdgePools, edgeTableSize );  
	...
	// Fill mFacePools table: for each face, a pool of smoothly touching faces and a pool of nonsmoothly touching ones.	
	AddZeroed(mFacePools, numPolygons );
	{for ( int p=0; p< numPolygons; p++ ) 
	{		
...
int CountCommonElements( std::vector<int>& ArrayOne, std::vector<int>& ArrayTwo )
{
	int Counter = 0;
...
int AnyCommonElements( std::vector<int>& ArrayOne, std::vector<int>& ArrayTwo )
{
...
void MeshProcessor::fillSmoothFaces( int polyid  )
	//PCF END
{		
	std::vector<int> TodoFaceStack;
	TodoFaceStack.push_back( polyid ); // Guaranteed to have been a NO_SMOOTHING_GROUP marked face.

	int LatestGroup = mCurrentGroup;

	while( TodoFaceStack.size() )
	{
		// Get top of stack.
		size_t StackTopIdx = TodoFaceStack.size() -1;
		int ThisFaceIdx = TodoFaceStack[ StackTopIdx ];		
		TodoFaceStack.erase(TodoFaceStack.begin() + StackTopIdx);
		...
		{for( int t=0; t< mFacePools[ ThisFaceIdx].HardFaces.size(); t++ )
		{
			int HardFaceAcross = mFacePools[ ThisFaceIdx].HardFaces[t] ;
			SharpSetCurrMatches += Contains(mFaceSmoothingGroups[ HardFaceAcross  ].Groups, mCurrentGroup );				
		}}
		...
		{			
			mFaceSmoothingGroups[ ThisFaceIdx ].Groups.push_back( mCurrentGroup );
			NewGroup = false;			
		}
		else
		{				
			...
			if( ( SharpSetCurrMatches > 0 )  && (SharpSetGroupMatches == 0) &&  (mFaceSmoothingGroups[ThisFaceIdx].Groups.size() > 0 ) ) 
			{
				NewGroup=false;
		...
		if( NewGroup )
		{
			...
			mFaceSmoothingGroups[ThisFaceIdx].Groups.push_back( SplashGroup ); // Set group in this face.
		}
		...
		{
			for( int t=0; t< mFacePools[ ThisFaceIdx ].SmoothFaces.size(); t++ )
			{
				...
				{
					AddUniqueItem(mFaceSmoothingGroups[ SmoothFaceAcross ].Groups, SplashGroup );
				}
			}
		}
		...
		for( int t=0; t< mFacePools[ ThisFaceIdx ].SmoothFaces.size(); t++ )
		{
			...
			{
				TodoFaceStack.push_back( SmoothFaceAcross );
...
void MeshProcessor::addEdgeInfo( int v1, int v2, bool smooth )
{
	...
	// Add it always.
	mVertEdgePools[v1].Edges.push_back( NewEdge );
}
...
MayaEdge* MeshProcessor::findEdgeInfo( int v1, int v2 )
{
	...
	EdgeIndex = 0;
	while( EdgeIndex < mVertEdgePools[v1].Edges.size() ) 
	{
	...
	EdgeIndex = 0;
	while( EdgeIndex < mVertEdgePools[v2].Edges.size() ) 
	{
