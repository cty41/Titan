...
int SceneTranslator::processStaticMesh(int TreeIndex)
{
	...
	if( mDoConvertSmooth && MeshHasMapping && ( !bCollision ) )
	{
		...
		if( TempTranslator.mFaceSmoothingGroups.size()  ) 
		{
			...
			mStaticPrimitives[PrimIndex].mFaceSmoothingGroups.insert(mStaticPrimitives[PrimIndex].mFaceSmoothingGroups.end(),
				TempTranslator.mFaceSmoothingGroups.size(),
				PrimGroupList());
	...
	for ( int i =0 ; i <  triMesh.triangles.size(); i++)
	{
		...
		//// A brand new face.
		int NewFaceIdx = mStaticPrimitives[PrimIndex].Faces.size() + 1;
		mStaticPrimitives[PrimIndex].Faces.push_back(GFace()); 
		mStaticPrimitives[PrimIndex].Faces[NewFaceIdx].MaterialIndices.push_back(0);
		...
		if( bHasSmoothingGroups )
		{
			...
			NewSmoothingGroups.push_back(GroupList());
	...
	if( OurScene.DoConvertSmooth && MeshHasMapping && ( !bCollision ) )
	{
		...
		if( TempTranslator.mFaceSmoothingGroups.size()  ) 
		{
			bSmoothingGroups = true;
			mStaticPrimitives[PrimIndex].mFaceSmoothingGroups.clear();
			AddExactZeroed(mStaticPrimitives[PrimIndex].mFaceSmoothingGroups, TempTranslator.mFaceSmoothingGroups.size());
	...
	{				
		...
		{							
			...
			AddExactZeroed(mStaticPrimitives[PrimIndex].Faces[NewFaceIdx].MaterialIndices, 1);
			mStaticPrimitives[PrimIndex].Faces[NewFaceIdx].MaterialIndices[0] = ThisMaterial;
	...
	if( bHasSmoothingGroups )
	{
		...
		mStaticPrimitives[PrimIndex].mFaceSmoothingGroups.clear();
		mStaticPrimitives[PrimIndex].mFaceSmoothingGroups.insert(
			mStaticPrimitives[PrimIndex].mFaceSmoothingGroups.end(), 
			NewSmoothingGroups.size(),
			PrimGroupList());
