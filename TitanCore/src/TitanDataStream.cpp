...
namespace Titan {
    ...
    String DataStream::getAsString(void)
    {
        ...
        char* pBuf = TITAN_ALLOC_T(char, mSize+1, MEMCATEGORY_GENERAL);
    ...
    void MemoryDataStream::close(void)    
    {
        ...
        {
            TITAN_FREE(mData, MEMCATEGORY_GENERAL);
            mData = 0;
        }
    ...
    void FileStreamDataStream::close(void)
    {
        if (mpInStream)
        {
            ...
            if (mFreeOnClose)
            {
				...
				if (mpFStreamRO)
					TITAN_DELETE_T(mpFStreamRO, basic_ifstream, MEMCATEGORY_GENERAL);
				if (mpFStream)
					TITAN_DELETE_T(mpFStream, basic_fstream, MEMCATEGORY_GENERAL);
				mpInStream = 0;
				mpFStreamRO = 0; 
				mpFStream = 0; 
            }
